...
def find_closest_name(user_input, auto_select = False):
    ...
    if auto_select:  #for testing, grab first match which should be the highest confidence one
        return matches[0][0]
...
def get_rec_by_name(game_name, min_players = None, max_players = None,
    ...
    recommended_games = recommended_games.copy()
    recommended_games['original_index'] = recommended_games.index  #preserves results indices for return & testing
    recommended_games = recommended_games.groupby('name_lower', as_index=False).first()

    #filter out repeated games in the same series using helper function / PRESERVE ORIGINAL INDEX!!
    filtered_ganes = []
    seen_roots = set()
    ...
    # Map similarity scores using the preserved original index
    recommended_games["similarity"] = recommended_games['original_index'].map(similarity_dict)
